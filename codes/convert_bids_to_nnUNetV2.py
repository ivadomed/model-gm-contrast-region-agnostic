    ...
    if os.path.exists(subject_image_file):
        if label_suffix is not None:
            if os.path.exists(subject_label_file):
                subject_label_file_nnunet = os.path.join(path_out_labels, f"{DS_name}-{sub_name}-{contrast}_{counter:03d}.nii.gz")
                ...
                subject_image_file_nnunet = os.path.join(path_out_images,
                                                         f"{DS_name}-{sub_name}-{contrast}_{counter:03d}_{channel:04d}.nii.gz")
